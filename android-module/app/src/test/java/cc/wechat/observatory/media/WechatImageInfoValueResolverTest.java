...
public final class WechatImageInfoValueResolverTest {
    ...
    @Test
    public void directRefTargetWinsBeforeProfileFileCandidate() throws Exception {
        File appRoot = Files.createTempDirectory("wxo-info-value-direct-ref").toFile();
        ...
        MediaResolver.ImageInfoResult result = WechatImageInfoValueResolver.resolve(
                appRoot,
    ...
    @Test
    public void profileFileWinsWhenDirectCandidateIsThumbnail() throws Exception {
        File appRoot = Files.createTempDirectory("wxo-info-value-profile-over-thumb").toFile();
        ...
        assertEquals(profileImage.getCanonicalFile(), result.source().getCanonicalFile());
    }

    @Test
    public void directUnsupportedIsReportedWhenNoCandidateNamesExist() throws Exception {
        File appRoot = Files.createTempDirectory("wxo-info-value-direct-unsupported").toFile();
        File unsupported = new File(appRoot, "not-image.bin");
        writeText(unsupported, "not an image");

        MediaResolver.ImageInfoResult result = WechatImageInfoValueResolver.resolve(
                appRoot,
                WechatImageInfoSnapshot.of(21L, Collections.singletonList(unsupported.getAbsolutePath()), null),
                null);

        assertEquals(MediaResolver.ImageInfoStatus.DIRECT_IMAGE_UNSUPPORTED, result.status());
        assertEquals(21L, result.localInfoId());
        assertEquals(unsupported.getCanonicalFile(), result.source().getCanonicalFile());
        assertNull(result.file());
    }

    @Test
    public void profileMissIsReportedBeforeDirectMissWhenNamesExist() throws Exception {
        File appRoot = Files.createTempDirectory("wxo-info-value-profile-miss").toFile();
        File unsupported = new File(appRoot, "not-image.bin");
        writeText(unsupported, "not an image");
        File pointer = image2File(appRoot, BASE + WechatImageFiles.REF_SUFFIX);
        writeText(pointer, REF_ID);
    ...
    @Test
    public void noCandidateNamesUsesEmptyDiagnosticsWhenNoImageInfoObjectExists() throws Exception {
        File appRoot = Files.createTempDirectory("wxo-info-value-no-candidate").toFile();

        MediaResolver.ImageInfoResult result = WechatImageInfoValueResolver.resolve(
                appRoot,
                WechatImageInfoSnapshot.of(23L, Collections.singletonList("<msg><img /></msg>"), null),
                null);

        assertEquals(MediaResolver.ImageInfoStatus.NO_CANDIDATE_NAMES, result.status());
        assertEquals(23L, result.localInfoId());
        assertTrue(result.candidateNames().isEmpty());
        assertTrue(result.fieldDebug().isEmpty());
    }

    @Test
    public void urlFieldDoesNotResolveThroughDirectOrProfileCandidatePaths() throws Exception {
        File appRoot = Files.createTempDirectory("wxo-info-value-url-skip").toFile();
        File normalized = new File(appRoot, "host/path/" + BASE + ".jpg");
        writeJpeg(normalized);

        MediaResolver.ImageInfoResult result = WechatImageInfoValueResolver.resolve(
                appRoot,
                WechatImageInfoSnapshot.of(
                        25L,
                        Collections.singletonList("https://host/path/" + BASE + ".jpg?download=1"),
                        null),
                null);

        assertEquals(MediaResolver.ImageInfoStatus.NO_CANDIDATE_NAMES, result.status());
        assertEquals(25L, result.localInfoId());
        assertNull(result.file());
        assertTrue(result.candidateNames().isEmpty());
    }
