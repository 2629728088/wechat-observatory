...
Wrapper 首次运行会下载 Gradle 分发包；离线环境可以使用 Android Studio，或配置本机 Gradle/`GRADLE_BIN` 运行项目验证。
...
| `poll_limit` | `20` | 每次最多拉取条数；小于 `outbox_parallelism` 时模块会按并发数提升批次 |
| `outbox_parallelism` | `4` | 出站并发 lane 数；不同 `wxid` 可并行，同一 `wxid` 保序 |
